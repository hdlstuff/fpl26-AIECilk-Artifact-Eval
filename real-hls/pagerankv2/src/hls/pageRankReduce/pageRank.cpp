...
void readVertex(void * mem0,
  ...
  for(int i = 0; i < v_size; i++){
    #pragma HLS PIPELINE II =1
    uint32_t uwide = MEM_IN(mem0, v_neighbours + i * sizeof(uint32_t), ap_uint<256>);
    // uint32_t u = uwide(31, 0);

    // Addr u_neighbors = MEM_IN(mem1, adj_list + (u << 4), Addr);

    uint32_t u_degree = MEM_IN(mem0, adj_list + (u << 4) + 8, ap_uint<256>);
    float Pcurr_u_wide = MEM_IN(mem1, Pcurr + u * sizeof(float), ap_uint<256>);
