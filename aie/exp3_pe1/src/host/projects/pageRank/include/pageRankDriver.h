...
public:
  pageRankDriver(std::vector<Memory *> memories, uint32_t delay, uint32_t depth, uint32_t branchFactor, uint32_t serialPhase)
      : mFpgaHardCilkDriver(memories), delay(delay), depth(depth), branchFactor(branchFactor), serialPhase(serialPhase) {}
  ...
  int run_test_bench_mFpga() override {
    ...
    int counter = std::pow(base_task.branchFactor - base_task.serialPhase, base_task.depth) * BASE_COUNT + 1;
