...
typedef struct {
    ...
    XVertex_map_uio_map maps[ MAX_UIO_MAPS ];
} XVertex_map_uio_info;

/***************** Variable Definitions **************************************/
static XVertex_map_uio_info uio_info;
...
static int uio_info_read_name(XVertex_map_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XVertex_map_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XVertex_map_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XVertex_map_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XVertex_map_Initialize(XVertex_map *InstancePtr, const char* InstanceName) {
	XVertex_map_uio_info *InfoPtr = &uio_info;
