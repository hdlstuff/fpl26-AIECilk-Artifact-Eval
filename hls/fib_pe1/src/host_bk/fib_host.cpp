...
void inline initializeScheduler(
  ...
  if (!base_task_data.empty()) {
    ...
    special_bo->sync(XCL_BO_SYNC_BO_FROM_DEVICE);
  }
  ...
  auto verify_map = special_bo->map<T *>();
  for (size_t i = 0; i < base_task_data.size(); i++) {
    if (std::memcmp(&verify_map[i + 1024 / sizeof(T) - 1], &base_task_data[i],
                    sizeof(T)) != 0) {
...
int main(int argc, char **argv) {
  ...
  std::cout << "Allocation successful for special_bo address "
            << special_bo->address() << std::endl;
  auto fib_task_bo = xrt::bo(device, buf_size * sizeof(fib_task), 0);

  std::cout << "Allocation successful for fib_task_bo address "
            << fib_task_bo.address() << std::endl;
